...
let conteudoEditCoress = document.getElementsByClassName(
  "conteudo_titulo_cores"
);
//clonando
let elClone = document.querySelector(".widget_clone");
let widgetChone = elClone.cloneNode(true);
console.log(widgetChone);
//menu mobile desktop
...
let spaceConteudo = document.querySelector(".space1");
...
for (let x = 0; x < delQtd; x++) {
  del[x].addEventListener("click", function () {
    ...
    let ulConteudo = widget[x].children[1].children;
    ...
    tituloCoresp.innerText = "(Novo) conteúdo";
    ulConteudo.innerHTML = `<li class="none">first</li>`;
    selecionarWidgetCores.value = "";
    selecionarWidgetInput.value = "";
    tituloCoresp.classList.remove(
      "c-cor1",
      "c-cor2",
      "c-cor3",
      "c-cor4",
...
for (let x = 0; x < widgetQtd; x++) {
  editTitulo[x].addEventListener("click", function () {
    let altenarIcone =
      widget[x].childNodes[1].childNodes[3].childNodes[1].childNodes;
...
for (let x = 0; x < widgetQtd; x++) {
  let containerWidgetCores =
    widget[x].childNodes[5].children[1].children[0].children;
  let tituloCoresp = widget[x].children[0].children[0];
